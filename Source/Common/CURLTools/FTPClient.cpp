...
//--	DESCRIPTION:
//--
//--		Implémentation de la classes FTPClient
...
namespace jhbCURLTools {
    ...
    #ifdef DEBUG_CURL
    std::string FTPClient::s_strCurlTraceLogDirectory;
    #endif
     ...
     *
     */
    //FTPClient::FTPClient(LogFnCallback Logger)
    FTPClient::FTPClient():
       strUserName_(""),
       strPassword_(""),
       strServer_(""),
       ...
       bNoSignal_(false),
       uPort_(0),
       eFtpProtocol_(FTP_PROTOCOL::FTP),
       eSettingsFlags_(SETTINGS_FLAG::ALL_FLAGS),
       strSSLCertFile_(""),
       strSSLKeyFile_(""),
       strSSLKeyPwd_(""),
    ...
    FTPClient::~FTPClient()
    {
       if (pCurlSession_ != nullptr) {
          //throw jhbCURLTools::CURLException(LOG_WARNING_OBJECT_NOT_CLEANED);
          CleanupSession();
       }
    }
     ...
     * @endcode
     */
    const void FTPClient::InitSession(const std::string &strHost, const unsigned &uPort, const std::string &strLogin,
                                       const std::string &strPassword, const FTP_PROTOCOL &eFtpProtocol /* = FTP */,
                                       const SETTINGS_FLAG& eSettingsFlags /* = ALL_FLAGS */)
    {
       if (strHost.empty()) {
           throw jhbCURLTools::CURLException(LOG_ERROR_EMPTY_HOST_MSG);
       }

       if (pCurlSession_) {
           throw jhbCURLTools::CURLException(LOG_ERROR_CURL_ALREADY_INIT_MSG);
       }

       if (nullptr == (pCurlSession_ = curl_easy_init())) {
           throw jhbCURLTools::CURLException(LOG_ERROR_CURL_NOT_INITIALIZED);
       }

       strServer_ = strHost;
       uPort_ = uPort;
       strUserName_ = strLogin;
       strPassword_ = strPassword;
       eFtpProtocol_   = eFtpProtocol;
       eSettingsFlags_ = eSettingsFlags;
    }
     ...
     * @endcode
     */
    const void FTPClient::CleanupSession() {
       if (!pCurlSession_) {
           throw jhbCURLTools::CURLException(LOG_ERROR_CURL_NOT_INIT_MSG);
       }
       ...
       curl_easy_cleanup(pCurlSession_);
       pCurlSession_ = nullptr;
    }
     ...
     *
     */
    void FTPClient::SetProgressFnCallback(void *pOwner, const ProgressFnCallback &fnCallback) {
       ProgressStruct_.pOwner = pOwner;
       fnProgressCallback_ = fnCallback;
       ProgressStruct_.pCurl = pCurlSession_;
     ...
     *
     */
    void FTPClient::SetProxy(const std::string &strProxy) {
     ...
     * @endcode
     */
    std::string FTPClient::ParseURL(const std::string &strRemoteFile) const {
       std::string strURL = strServer_ + "/" + strRemoteFile;
       ...
       if (strUri.compare(0, 3, "FTP") != 0 && strUri.compare(0, 4, "SFTP") != 0) {
          switch (eFtpProtocol_) {
             case FTP_PROTOCOL::FTP:
             default:
                 completeURL = IPPROTO_FTP;
                break;

             case FTP_PROTOCOL::FTPS:
                 completeURL = IPPROTO_FTPS;
                break;

             case FTP_PROTOCOL::FTPES:
                 completeURL = IPPROTO_FTPES;
                break;

             case FTP_PROTOCOL::SFTP:
                 completeURL = IPPROTO_SFTP;
                break;
          }
       }

       completeURL += IPPROTO_URL_SEP;
       completeURL += strURL;
       return completeURL;
    }
     ...
     * @endcode
     */
    const void FTPClient::CreateDir(const std::string &strNewDir) const {
        if (strNewDir.empty()) {
            throw jhbCURLTools::CURLException(LOG_ERROR_INVALID_PARAMETERS);
        }
       ...
       // Specify target
       curl_easy_setopt(pCurlSession_, CURLOPT_URL, strRemoteFolder.c_str());
       ...
       curl_easy_setopt(pCurlSession_, CURLOPT_POSTQUOTE, headerlist);
       curl_easy_setopt(pCurlSession_, CURLOPT_NOBODY, 1L);
       curl_easy_setopt(pCurlSession_, CURLOPT_HEADER, 1L);
       curl_easy_setopt(pCurlSession_, CURLOPT_FTP_CREATE_MISSING_DIRS, CURLFTP_CREATE_DIR);
       ...
       // Check for errors
       if (res != CURLE_OK) {
          std::string message(StringFormat(LOG_ERROR_CURL_MKDIR_FORMAT, strRemoteNewFolderName.c_str(), res, curl_easy_strerror(res)));
          throw jhbCURLTools::CURLException(message);
       }
    }
     ...
     * @endcode
     */
    const bool FTPClient::RemoveDir(const std::string &strDir) const {
        if (strDir.empty()) {
            throw jhbCURLTools::CURLException(LOG_ERROR_INVALID_PARAMETERS);
            return false;
        }
       ...
       if (res != CURLE_OK) {
           //throw jhbCURLTools::CURLException(StringFormat(LOG_ERROR_CURL_RMDIR_FORMAT, strRemoteFolderName.c_str(), res, curl_easy_strerror(res)));
           return false;
       }
     ...
     * @endcode
     */
    const void FTPClient::RemoveFile(const std::string &strRemoteFile) const {
       ...
       // Check for errors
       if (res != CURLE_OK) {
           std::string message(StringFormat(LOG_ERROR_CURL_REMOVE_FORMAT, strRemoteFile.c_str(), res, curl_easy_strerror(res)));
           throw jhbCURLTools::CURLException(message);
       }
    }
     ...
     * @endcode
     */
    const bool FTPClient::Info(const std::string& strRemoteFile, FTPFILEINFO& oFileInfo) const {
        if (strRemoteFile.empty()) {
            throw jhbCURLTools::CURLException(LOG_ERROR_INVALID_PARAMETERS);
            return false;
        }
        ...
        curl_easy_setopt(pCurlSession_, CURLOPT_URL, ParseURL(strRemoteFile).c_str());

        /* No download if the file */
        curl_easy_setopt(pCurlSession_, CURLOPT_NOBODY, 1L);

        /* Ask for filetime */
        curl_easy_setopt(pCurlSession_, CURLOPT_FILETIME, 1L);

        /* No header output: TODO 14.1 http-style HEAD output for ftp */
        curl_easy_setopt(pCurlSession_, CURLOPT_HEADERFUNCTION, ThrowAwayCallback);
        curl_easy_setopt(pCurlSession_, CURLOPT_HEADER, 0L);
        ...
        if (CURLE_OK == res) {
            ...
            res = curl_easy_getinfo(pCurlSession_, CURLINFO_FILETIME, &lFileTime);
            if (CURLE_OK == res && lFileTime >= 0) {
                oFileInfo.tFileMTime = static_cast<time_t>(lFileTime);
            }

            res = curl_easy_getinfo(pCurlSession_, CURLINFO_CONTENT_LENGTH_DOWNLOAD, &oFileInfo.dFileSize);
        ...
        /*
            ...
            throw jhbCURLTools::CURLException(StringFormat(LOG_ERROR_CURL_FILETIME_FORMAT, strRemoteFile.c_str(), res, curl_easy_strerror(res)));
        }
        */
     ...
     * @endcode
     */
    const void FTPClient::List(const std::string &strRemoteFolder, std::string &strList, bool bOnlyNames /* = true */) const {
       ...
       curl_easy_setopt(pCurlSession_, CURLOPT_URL, strRemoteFile.c_str());

       if (bOnlyNames) curl_easy_setopt(pCurlSession_, CURLOPT_DIRLISTONLY, 1L);

       curl_easy_setopt(pCurlSession_, CURLOPT_WRITEFUNCTION, WriteInStringCallback);
       curl_easy_setopt(pCurlSession_, CURLOPT_WRITEDATA, &strList);

       CURLcode res = Perform();

       if (CURLE_OK != res) {
           std::string message(StringFormat(LOG_ERROR_CURL_FILELIST_FORMAT, strRemoteFolder.c_str(), res, curl_easy_strerror(res)));
           throw jhbCURLTools::CURLException(message);
       }
    }
     ...
     * @endcode
     */
    const void FTPClient::DownloadFile(const std::string &strLocalFile, const std::string &strRemoteFile) const {
        ...
        if (ofsOutput) {
            curl_easy_setopt(pCurlSession_, CURLOPT_URL, strFile.c_str());
            curl_easy_setopt(pCurlSession_, CURLOPT_WRITEFUNCTION, WriteToFileCallback);
            curl_easy_setopt(pCurlSession_, CURLOPT_WRITEDATA, &ofsOutput);
            ...
            if (res != CURLE_OK) {
                ...
                std::string message(StringFormat(LOG_ERROR_CURL_GETFILE_FORMAT, strServer_.c_str(), strRemoteFile.c_str(), res, curl_easy_strerror(res)));
                throw jhbCURLTools::CURLException(message);
            }
            ...
            throw jhbCURLTools::CURLException(LOG_ERROR_FILE_GETFILE_FORMAT);
        }
    }
     ...
     *
     */
    const void FTPClient::DownloadFile(const std::string &strRemoteFile, std::vector<char> &data) const {
        if (strRemoteFile.empty()) {
            throw jhbCURLTools::CURLException(LOG_ERROR_INVALID_PARAMETERS);
        }
       ...
       curl_easy_setopt(pCurlSession_, CURLOPT_URL, strFile.c_str());
       curl_easy_setopt(pCurlSession_, CURLOPT_WRITEFUNCTION, WriteToMemory);
       curl_easy_setopt(pCurlSession_, CURLOPT_WRITEDATA, &data);
     ...
     * @endcode
     */
    const void FTPClient::DownloadWildcard(const std::string &strLocalDir, const std::string &strRemoteWildcard) const {
       ...
       if (stat(data.strOutputPath.c_str(), &info) == 0 && (info.st_mode & S_IFDIR))  // S_ISDIR() doesn't exist on windows
       {
          ...
          curl_easy_setopt(pCurlSession_, CURLOPT_WILDCARDMATCH, 1L);

          /* 1. callback is called before download of concrete file started */
          curl_easy_setopt(pCurlSession_, CURLOPT_CHUNK_BGN_FUNCTION, FileIsComingCallback);

          /* 2. this callback will write contents into files */
          curl_easy_setopt(pCurlSession_, CURLOPT_WRITEFUNCTION, WriteItCallback);

          /* 3. callback is called after data from the file have been transferred */
          curl_easy_setopt(pCurlSession_, CURLOPT_CHUNK_END_FUNCTION, FileIsDownloadedCallback);

          /* put transfer data into callbacks */
          curl_easy_setopt(pCurlSession_, CURLOPT_CHUNK_DATA, &data);
          curl_easy_setopt(pCurlSession_, CURLOPT_WRITEDATA, &data);
          /* curl_easy_setopt(pCurlSession_, CURLOPT_VERBOSE, 1L); */

          /* set an URL containing wildcard pattern (only in the last part) */
          curl_easy_setopt(pCurlSession_, CURLOPT_URL, strPattern.c_str());
          ...
          if (res != CURLE_OK && res != CURLE_REMOTE_FILE_NOT_FOUND) {
              std::string message(StringFormat(LOG_ERROR_CURL_GETWILD_FORMAT, strServer_.c_str(), strRemoteWildcard.c_str(), res, curl_easy_strerror(res)));
              throw jhbCURLTools::CURLException(message);
          }
          /* folders need to be copied integrally */
            ...
            for (const auto &Dir : data.vecDirList) {
                DownloadWildcard(data.strOutputPath + Dir, strBaseUrl + Dir + "/*");
             }
          }
       }
       else {
           std::string message(StringFormat(LOG_ERROR_DIR_GETWILD_FORMAT, data.strOutputPath.c_str()));
           throw jhbCURLTools::CURLException(message);
       }
    }
     ...
     * @endcode
     */
    const void FTPClient::UploadFile(const std::string &strLocalFile, const std::string &strRemoteFile, const bool &bCreateDir) const {
        if (strLocalFile.empty() || strRemoteFile.empty()) {
            throw jhbCURLTools::CURLException(LOG_ERROR_INVALID_PARAMETERS);
        }
       ...
       if (stat(strLocalFile.c_str(), &file_info) == 0) {
           ...
           if (!InputFile) {
               std::string message(StringFormat(LOG_ERROR_FILE_UPLOAD_FORMAT, strLocalFile.c_str()));
               throw jhbCURLTools::CURLException(message);
           }

           /* specify target */
           curl_easy_setopt(pCurlSession_, CURLOPT_URL, strLocalRemoteFile.c_str());

           /* we want to use our own read function */
           curl_easy_setopt(pCurlSession_, CURLOPT_READFUNCTION, ReadFromFileCallback);

           /* now specify which file to upload */
           curl_easy_setopt(pCurlSession_, CURLOPT_READDATA, &InputFile);

           /* Set the size of the file to upload (optional).  If you give a *_LARGE
           option you MUST make sure that the type of the passed-in argument is a
           curl_off_t. If you use CURLOPT_INFILESIZE (without _LARGE) you must
           make sure that to pass in a type 'long' argument. */
           curl_easy_setopt(pCurlSession_, CURLOPT_INFILESIZE_LARGE, static_cast<curl_off_t>(file_info.st_size));

           /* enable uploading */
           curl_easy_setopt(pCurlSession_, CURLOPT_UPLOAD, 1L);
           ...
           if (res != CURLE_OK) {
               std::string message(StringFormat(LOG_ERROR_CURL_UPLOAD_FORMAT, strLocalFile.c_str(), res, curl_easy_strerror(res)));
               throw jhbCURLTools::CURLException(message);
           }
     ...
     *
     */
    const CURLcode FTPClient::Perform() const {
       CURLcode res = CURLE_OK;

       curl_easy_setopt(pCurlSession_, CURLOPT_USERPWD, (strUserName_ + ":" + strPassword_).c_str());

       if (!strProxy_.empty()) curl_easy_setopt(pCurlSession_, CURLOPT_FTPPORT, uPort_);

       if (iCurlTimeout_ > 0) {
          curl_easy_setopt(pCurlSession_, CURLOPT_TIMEOUT, iCurlTimeout_);
          // don't want to get a sig alarm on timeout
          curl_easy_setopt(pCurlSession_, CURLOPT_NOSIGNAL, 1);
       }

       if (!strProxy_.empty()) {
          curl_easy_setopt(pCurlSession_, CURLOPT_PROXY, strProxy_.c_str());
          curl_easy_setopt(pCurlSession_, CURLOPT_HTTPPROXYTUNNEL, 1L);

          // use only plain PASV
          if (!bActive_) {
             curl_easy_setopt(pCurlSession_, CURLOPT_FTP_USE_EPSV, 1L);
          }
       }
       ...
       if (eFtpProtocol_ == FTP_PROTOCOL::FTPS || eFtpProtocol_ == FTP_PROTOCOL::FTPES)
          ...
          curl_easy_setopt(pCurlSession_, CURLOPT_USE_SSL, CURLUSESSL_ALL);

       if (eFtpProtocol_ == FTP_PROTOCOL::SFTP
           && ((int)eSettingsFlags_ & (int)SETTINGS_FLAG::ENABLE_SSH)
           )
          /* We activate ssh agent. For this to work you need
          to have ssh-agent running (type set | grep SSH_AGENT to check) or
          pageant on Windows (there is an icon in systray if so) */
          curl_easy_setopt(pCurlSession_, CURLOPT_SSH_AUTH_TYPES, CURLSSH_AUTH_AGENT);

       // SSL
       if (!strSSLCertFile_.empty()) {
           curl_easy_setopt(pCurlSession_, CURLOPT_SSLCERT, strSSLCertFile_.c_str());
       }

       if (!strSSLKeyFile_.empty()) {
           curl_easy_setopt(pCurlSession_, CURLOPT_SSLKEY, strSSLKeyFile_.c_str());
       }

       if (!strSSLKeyPwd_.empty()) {
           curl_easy_setopt(pCurlSession_, CURLOPT_KEYPASSWD, strSSLKeyPwd_.c_str());
       }
       ...
       res = curl_easy_perform(pCurlSession_);
     ...
     */
    std::string FTPClient::StringFormat(std::string strFormat, ...) {
       va_list args;
    ...
    void FTPClient::ReplaceString(std::string &strSubject, const std::string &strSearch, const std::string &strReplace) {
    ...
    size_t FTPClient::ThrowAwayCallback(void *ptr, size_t size, size_t nmemb, void *data) {
     ...
     * @return (size * nmemb)
     */
    size_t FTPClient::WriteInStringCallback(void *ptr, size_t size, size_t nmemb, void *data) {
     ...
     * @return (size * nmemb)
     */
    size_t FTPClient::WriteToFileCallback(void *buff, size_t size, size_t nmemb, void *data) {
     ...
     * @return (size * nmemb)
     */
    size_t FTPClient::WriteToMemory(void *buff, size_t size, size_t nmemb, void *data) {
       if ((size == 0) || (nmemb == 0) || ((size * nmemb) < 1) || (data == nullptr)) return 0;
     ...
     * @return (size * nmemb)
     */
    size_t FTPClient::ReadFromFileCallback(void *ptr, size_t size, size_t nmemb, void *stream) {
       std::ifstream *pFileStream = reinterpret_cast<std::ifstream *>(stream);
       if (pFileStream->is_open()) {
          pFileStream->read(reinterpret_cast<char *>(ptr), size * nmemb);
          return (size_t)pFileStream->gcount();
       }
       return 0;
    }
     ...
     * (continue)
     */
    long FTPClient::FileIsComingCallback(struct curl_fileinfo *finfo, WildcardTransfersCallbackData *data, int remains) {
     ...
     */
    long FTPClient::FileIsDownloadedCallback(WildcardTransfersCallbackData *data) {
       if (data->ofsOutput.is_open()) {
          // printf("DOWNLOADED\n");
     ...
     */
    size_t FTPClient::WriteItCallback(char *buff, size_t size, size_t nmemb, void *cb_data) {
    ...
    //
    void FTPClient::SetCurlTraceLogDirectory(const std::string &strPath) {
    ...
    int FTPClient::DebugCallback(CURL *curl, curl_infotype curl_info_type, char *pszTrace, size_t usSize, void *pFile) {
       std::string strText;
    ...
    void FTPClient::StartCurlDebug() const {
       if (!m_ofFileCurlTrace.is_open()) {
          curl_easy_setopt(pCurlSession_, CURLOPT_VERBOSE, 1L);
          curl_easy_setopt(pCurlSession_, CURLOPT_DEBUGFUNCTION, DebugCallback);
          ...
          if (!strFileCurlTraceFullName.empty()) {
             ...
             if (m_ofFileCurlTrace) curl_easy_setopt(pCurlSession_, CURLOPT_DEBUGDATA, &m_ofFileCurlTrace);
          }
       }
    }

    void FTPClient::EndCurlDebug() const {
